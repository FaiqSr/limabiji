...
<div class="space-y-6">
    ...
    <div class="card-modern flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Manage specialty coffee partner regions, cupping scores, altitudes, and processing methods.</p>
        </div>

        <a href="{{ route('admin.origins.create') }}" class="btn btn-primary text-xs py-2 px-4 shadow-2xs self-start sm:self-auto shrink-0 w-full sm:w-fit">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        @forelse ($origins as $origin)
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-2xs hover:shadow-md hover:border-slate-300 transition-all duration-200 flex flex-col group">
                ...
                <div class="relative aspect-16/9 bg-slate-100 overflow-hidden">
                    ...
                    <!-- Gradient Overlay -->
                    <div class="absolute inset-0 bg-gradient-to-t from-slate-900/70 via-transparent to-black/20 pointer-events-none"></div>
                    ...
                    <div class="absolute top-3 left-3">
                        ...
                        @else
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold bg-slate-800/80 text-slate-200 shadow-xs backdrop-blur-xs">
                    ...
                    @if ($origin->score)
                        <div class="absolute top-3 right-3">
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-mono font-bold bg-white/95 text-slate-900 shadow-xs backdrop-blur-xs border border-white/40">
                    ...
                    @if ($galleryCount > 0)
                        <div class="absolute bottom-3 right-3">
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-semibold bg-slate-900/75 text-white backdrop-blur-xs">
                ...
                <div class="p-5 flex-1 flex flex-col justify-between space-y-4">
                    <div class="space-y-2.5">
                        ...
                        <div class="flex flex-wrap gap-1.5 pt-1">
                            @if ($origin->altitude)
                                <span class="inline-flex items-center gap-1 text-[11px] font-mono px-2 py-0.5 rounded-md bg-slate-50 text-slate-600 border border-slate-200">
                            ...
                            @if ($origin->process)
                                <span class="inline-flex items-center text-[11px] font-medium px-2 py-0.5 rounded-md bg-indigo-50 text-indigo-700 border border-indigo-100">
                                    {{ $origin->process }}
                                </span>
                            @endif

                            @if ($origin->varietals)
                                <span class="inline-flex items-center text-[11px] font-medium px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 border border-slate-200 truncate max-w-[200px]" title="{{ $origin->varietals }}">
                                    {{ $origin->varietals }}
                                </span>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-slate-100 gap-2">
                        ...
                        <div class="flex items-center gap-1.5">
                            ...
                            <form action="{{ route('admin.origins.destroy', $origin) }}" 
                                  method="POST" 
        ...
        @empty
            <div class="col-span-full card-modern text-center py-16">
                ...
                <p class="text-xs text-slate-500 mt-1 max-w-sm mx-auto">Get started by creating your first regional coffee origin profile.</p>
                <div class="mt-4">
                    <a href="{{ route('admin.origins.create') }}" class="btn btn-primary text-xs py-2 px-4 shadow-2xs inline-flex items-center gap-1.5">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                        </svg>
