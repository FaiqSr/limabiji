...
@section('page_title', 'Edit FAQ: ' . Str::limit($faq->question, 40))

@section('content')
<form action="{{ route('admin.faqs.update', $faq) }}" method="POST" x-data="{ locale: 'en' }" class="max-w-4xl mx-auto space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="card-modern space-y-6">
        ...
        <div x-show="locale === 'en'" class="space-y-4">
            <div>
                ...
                <input type="text" 
                       ...
                       class="w-full text-xs bg-white border @error('question') border-rose-500 @else border-slate-200 @enderror rounded-xl p-3 focus:ring-2 focus:ring-indigo-500 transition-all">
                @error('question')
            ...
            <div>
                ...
                <textarea name="answer" 
                          ...
                          class="w-full text-xs bg-white border @error('answer') border-rose-500 @else border-slate-200 @enderror rounded-xl p-3 focus:ring-2 focus:ring-indigo-500 leading-relaxed transition-all">{{ old('answer', $faq->answer) }}</textarea>
                @error('answer')
                    <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div x-show="locale === 'id'" style="display: none;" class="space-y-4">
            <div>
                ...
                <input type="text" 
                       ...
                       class="w-full text-xs bg-white border @error('question_id') border-rose-500 @else border-slate-200 @enderror rounded-xl p-3 focus:ring-2 focus:ring-indigo-500 transition-all">
                @error('question_id')
                    <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="answer_id" 
                          ...
                          class="w-full text-xs bg-white border @error('answer_id') border-rose-500 @else border-slate-200 @enderror rounded-xl p-3 focus:ring-2 focus:ring-indigo-500 leading-relaxed transition-all">{{ old('answer_id', $faq->answer_id) }}</textarea>
                @error('answer_id')
                    <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="pt-4 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
            ...
            <div>
                ...
                <input type="number" 
                       ...
                       min="0" 
                       class="w-full text-xs bg-white border @error('order') border-rose-500 @else border-slate-200 @enderror rounded-xl p-2.5 focus:ring-2 focus:ring-indigo-500">
        ...
        <div class="pt-2 flex items-center justify-between gap-4">
            <button type="submit" class="btn btn-primary flex-1 py-3 text-sm font-semibold shadow-xs">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                </svg>
                <span>Update FAQ Item</span>
            </button>
        </div>
    </div>
</form>
