    ...
    <div class="max-w-lg">
        <div class="card-modern space-y-4">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                ...
                <a href="{{ route('admin.users.index') }}" class="text-xs text-slate-500 hover:text-indigo-600 font-medium flex items-center gap-1">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required class="w-full text-xs bg-white border @error('name') border-rose-500 @else border-slate-200 @enderror rounded-lg p-2.5 focus:ring-2 focus:ring-indigo-500">
                @error('name')
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" required class="w-full text-xs bg-white border @error('email') border-rose-500 @else border-slate-200 @enderror rounded-lg p-2.5 focus:ring-2 focus:ring-indigo-500">
                @error('email')
                    <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="role" id="role" required class="w-full text-xs bg-white border @error('role') border-rose-500 @else border-slate-200 @enderror rounded-lg p-2.5 focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
                ...
                <input type="password" name="password" id="password" minlength="8" {{ $user->exists ? '' : 'required' }} placeholder="••••••••" class="w-full text-xs bg-white border @error('password') border-rose-500 @else border-slate-200 @enderror rounded-lg p-2.5 focus:ring-2 focus:ring-indigo-500">
                @error('password')
                    <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                @enderror
            </div>
