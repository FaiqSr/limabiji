...
@section('page_title', 'Create Store Product')
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-base font-bold text-slate-900">Add New Coffee Product</h3>
                <p class="text-xs text-slate-500">Configure beans profile, multi-weight pricing, and storefront assets.</p>
            </div>
        </div>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.products.store') }}" method="POST" class="space-y-6" x-data="{
        name: '{{ old('name', '') }}',
        slug: '{{ old('slug', '') }}',
        generateSlug() {
        ...
        tastingNotes: {{ json_encode(old('tasting_notes', [])) }},
        newNote: '',
        addNote() {
        ...
        flavorTags: {{ json_encode(old('flavor_tags', [])) }},
        newTag: '',
        addTag() {
            if (this.newTag.trim()) {
                ...
                <div class="card-modern space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="name" name="name" x-model="name" @input="generateSlug" value="{{ old('name') }}" placeholder="e.g. Gayo Honey Civet" required>
                        </div>
                        <div>
                            <label for="name_id">Product Name (ID)</label>
                            <input type="text" id="name_id" name="name_id" value="{{ old('name_id') }}" placeholder="e.g. Gayo Honey Luwak">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex rounded-lg border border-slate-200 overflow-hidden focus-within:border-slate-900 focus-within:ring-2 focus-within:ring-slate-900/10 transition-all bg-white shadow-2xs">
                            ...
                            <input type="text" id="slug" name="slug" x-model="slug" value="{{ old('slug') }}" class="border-0 focus:ring-0 focus:outline-none font-mono text-xs px-3 py-2 flex-1" placeholder="product-slug" required>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <select id="roast_level" name="roast_level" required>
                                @foreach ($roastLevels as $rl)
                                    <option value="{{ $rl }}" {{ old('roast_level', 'medium') === $rl ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3" placeholder="Sensory characteristics, cup profile...">{{ old('description') }}</textarea>
                    </div>

                    <div>
                        <label for="description_id">Description (ID)</label>
                        <textarea id="description_id" name="description_id" rows="3" placeholder="Karakteristik rasa, profil seduhan...">{{ old('description_id') }}</textarea>
                    </div>
                </div>
                ...
                <div class="card-modern space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="text" id="origin" name="origin" value="{{ old('origin') }}" placeholder="e.g. Takengon, Aceh">
                        </div>
                        <div>
                            <label for="altitude">Altitude</label>
                            <input type="text" id="altitude" name="altitude" value="{{ old('altitude') }}" placeholder="e.g. 1,450 - 1,600 MASL">
                        </div>
                        <div>
                            <label for="process">Processing Method</label>
                            <input type="text" id="process" name="process" value="{{ old('process') }}" placeholder="e.g. Natural Anaerobic">
                        </div>
                    </div>

                    <div>
                        <label for="sca_score">SCA Cupping Score</label>
                        <input type="text" id="sca_score" name="sca_score" value="{{ old('sca_score') }}" placeholder="e.g. 88.5" class="font-mono">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            @php
                                ...
                                $selectedBrews = old('recommended_brews', ['v60', 'espresso']);
                            @endphp
            ...
            <div class="space-y-6">
                ...
                <div class="card-modern space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex rounded-lg border border-slate-200 overflow-hidden focus-within:border-slate-900 focus-within:ring-2 focus-within:ring-slate-900/10 transition-all bg-white shadow-2xs">
                            ...
                            <input type="number" id="base_price_200g" name="base_price_200g" value="{{ old('base_price_200g', 75000) }}" class="border-0 focus:ring-0 focus:outline-none font-mono text-xs px-3 py-2 flex-1" min="0" required>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex rounded-lg border border-slate-200 overflow-hidden focus-within:border-slate-900 focus-within:ring-2 focus-within:ring-slate-900/10 transition-all bg-white shadow-2xs">
                            <span class="inline-flex items-center px-3 bg-slate-50 text-slate-500 font-mono text-xs border-r border-slate-200 select-none">Rp</span>
                            <input type="number" id="price_500g" name="price_500g" value="{{ old('price_500g', 175000) }}" class="border-0 focus:ring-0 focus:outline-none font-mono text-xs px-3 py-2 flex-1" min="0" required>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex rounded-lg border border-slate-200 overflow-hidden focus-within:border-slate-900 focus-within:ring-2 focus-within:ring-slate-900/10 transition-all bg-white shadow-2xs">
                            <span class="inline-flex items-center px-3 bg-slate-50 text-slate-500 font-mono text-xs border-r border-slate-200 select-none">Rp</span>
                            <input type="number" id="price_1kg" name="price_1kg" value="{{ old('price_1kg', 320000) }}" class="border-0 focus:ring-0 focus:outline-none font-mono text-xs px-3 py-2 flex-1" min="0" required>
                        </div>
                    </div>

                    <div>
                        <label for="stock">Current Stock (units) <span class="text-rose-500">*</span></label>
                        <input type="number" id="stock" name="stock" value="{{ old('stock', 100) }}" min="0" required class="font-mono text-xs">
                    </div>
                </div>
                ...
                <div class="card-modern space-y-4">
                    ...
                    <label class="flex items-center gap-3 p-3 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 cursor-pointer select-none">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-4 h-4 text-emerald-600 rounded">
                        <div>
                    ...
                    <label class="flex items-center gap-3 p-3 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 cursor-pointer select-none">
                        <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', false) ? 'checked' : '' }} class="w-4 h-4 text-amber-600 rounded">
                        <div>
