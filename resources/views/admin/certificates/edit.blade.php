...
@section('content')
<form action="{{ route('admin.certificates.update', $certificate) }}" method="POST" enctype="multipart/form-data" class="max-w-4xl mx-auto space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="card-modern space-y-6">
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
            <div>
                <h3 class="text-base font-semibold text-slate-900">Edit Certificate Details</h3>
                <p class="text-xs text-slate-500 mt-0.5">Update certification information and public display status.</p>
            </div>
            @if($certificate->logo)
                <div class="w-12 h-12 rounded-xl bg-slate-100 border border-slate-200 p-1 flex items-center justify-center overflow-hidden">
                    <img src="{{ $certificate->logo }}" alt="{{ $certificate->name }}" class="w-full h-full object-cover rounded-lg">
                </div>
            @endif
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input type="url" 
                       id="logo" 
                       name="logo" 
                       value="{{ old('logo', $certificate->logo) }}" 
                       placeholder="https://..."
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="description" 
                          ...
                          class="w-full text-xs bg-white border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-emerald-500 leading-relaxed transition-all">{{ old('description', $certificate->description) }}</textarea>
            </div>
        </div>
