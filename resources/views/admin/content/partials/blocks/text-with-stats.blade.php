<div class="space-y-3">
    ...
    <div>
        <x-cms.field-header label="Paragraph 1" field="body" :autoTranslate="true" />
    ...
    <div>
        <x-cms.field-header label="Paragraph 2" field="body2" :autoTranslate="true" />
        <textarea :id="'block-' + index + '-' + loc + '-body2'"
                  :name="'blocks['+index+'][content]['+loc+'][body2]'" 
    ...
    <div>
        <x-cms.field-header label="Checklist Items (comma-separated)" field="checklist" :autoTranslate="true" />
        <input type="text" 
               :id="'block-' + index + '-' + loc + '-checklist'"
               :name="'blocks['+index+'][content]['+loc+'][checklist]'" 
               class="text-xs w-full border border-slate-200 rounded-lg p-2 focus:ring-2 focus:ring-indigo-500" 
               x-model="content(block, loc).checklist" 
               @input="isDirty = true; queuePreviewUpdate()">
    </div>
    <div class="space-y-2">
        <label class="text-xs font-semibold text-slate-700">Stats (Repeater)</label>
        <template x-for="(item, itemIndex) in items(block, loc)" :key="itemIndex">
            <div class="grid grid-cols-[1fr_1fr_auto] gap-2 items-center bg-slate-50 p-2 rounded-lg border border-slate-200">
                <input type="text" 
                       :id="'block-' + index + '-' + loc + '-tws-l-' + itemIndex"
                ...
                <input type="text" 
                       :id="'block-' + index + '-' + loc + '-tws-v-' + itemIndex"
        ...
        <button type="button" 
                ...
                class="btn btn-secondary py-1 px-3 text-xs">+ Add Stat</button>
    </div>
</div>
